...
export function toString_no_rebuttal(review) {
    return toString(review.values);
}

export function toString_rebuttal(review) {
    let ret = toString(review.values);
    for (const rebuttal of review.rebuttal) {
        ...
        if (rebuttal.comments && rebuttal.comments.length > 0) {
            for (const comment of rebuttal.comments) {
                ret += "REPLY \n" + comment.comment + "\n\n";
            }
        }
    }
...
export function toString(metareview, conference='ICLR') {
    ...
    for (let field of fields) {
        if (metareview[field] && metareview[field] !== 'not_provided') {
            ret += field;
            if (numeric_fields.includes(field)) {
                if (field === 'rating' || field === 'overall_recommendation') {
                    ret += ': ' + ratingScores[metareview[field]];
                } else if (field === 'confidence') {
                    ret += ': ' + confidenceScores[metareview[field]];
                } else if (field === 'correctness') {
                    ret += ': ' + correctnessScores[metareview[field]];
                } else if (field === 'technical novelty and significance') {
                    ret += ': ' + technicalNoveltyAndSignificanceScores[metareview[field]];
                } else if (field === 'empirical novelty and significance') {
                    ret += ': ' + empiricalNoveltyAndSignificanceScores[metareview[field]];
                } else {
                    ret += ': ' + miscScores[metareview[field]];
                }
            } else {
                ret += ": " + metareview[field];
            }
            ret += "\n\n";
        }
    }
